...
class LigandProcessor(object):
    def __init__(self, smiles_string, sdf_path, mol_name=None, n_cpu=1, enum_isomer=True,
                 max_conformer_per_isomer=500, max_attempts=1000, remove_twisted6ring=True,
                 ignore=3, prot=False, verbose=False):
    ...
    def __correct_cis_amide__(self, mol, conformation_indices):
        pattern = Chem.MolFromSmarts('[*][NH1]-;!@C=O')
        matches = mol.GetSubstructMatches(pattern)

        for conformation_idx in conformation_indices:
            for a1, a2, a3, a4 in matches:
                if mol.GetAtomWithIdx(a1).GetSymbol() != 'H':
                    rdMolTransforms.SetDihedralDeg(mol.GetConformer(conformation_idx), a1, a2, a3, a4, 0)
    ...
    def __eliminate_twisted_six_membered_ring__(self, mol):
        ...
        six_membered_ring_indices = [atom_rings_idx for atom_rings_idx, atom_rings in enumerate(all_atom_rings) if len(atom_rings) == 6]
        aliphatic_ring_indices = [atom_rings_idx for atom_rings_idx, atom_rings in enumerate(all_atom_rings) if not mol.GetAtomWithIdx(atom_rings[0]).GetIsAromatic()]

        sp3_simple_six_membered_ring_indices = []

        for six_membered_ring_idx in six_membered_ring_indices:
            isolated_sp2_in_ring = True
            six_membered_atom_ring = all_atom_rings[six_membered_ring_idx]
            sp2_atom_indices = []
            ...
            for atom_idx in six_membered_atom_ring:
                ...
                if current_atom.GetHybridization() != Chem.rdchem.HybridizationType.SP3:
                    for other_atom_idx in sp2_atom_indices:
                        ...
                        if current_bond:
                            isolated_sp2_in_ring = False
                            break
                    sp2_atom_indices.append(atom_idx)

            if isolated_sp2_in_ring and len(sp2_atom_indices) <= 2:
                bridged_ring = False
                for aliphatic_ring_idx in aliphatic_ring_indices:
                    if six_membered_ring_idx == aliphatic_ring_idx:
                        continue
                    aliphatic_atom_ring = all_atom_rings[aliphatic_ring_idx]

                    if len(set(six_membered_atom_ring).intersection(set(aliphatic_atom_ring))) > 2:
                        bridged_ring = True
                        break
    ...
    def __get_nonplanar_ring_with_substituents__(self, mol):
        _, torsion_list_rings = TorsionFingerprints.CalculateTorsionLists(mol)
    ...
    def __write_conformers_to_sdf__(self, sdf_path, mol, conf_indices, conformer_energies, mol_name, mol_idx):
        if mol_name is None:
            saved_sdf_file_name = os.path.join(sdf_path, get_rand_id() + '.sdf')
        else:
            saved_sdf_file_name = os.path.join(sdf_path, '%s_%04d.sdf' %(mol_name, mol_idx))

        sdf_writer = Chem.SDWriter(saved_sdf_file_name)
        for i, conf_idx in enumerate(conf_indices):
            for prop_name in mol.GetPropNames():
                mol.ClearProp(prop_name)

            mol.SetIntProp('conformer_id', i)
            mol.SetProp('conformer_energy', str(conformer_energies[conf_idx][1]))
            sdf_writer.write(mol, confId=conf_idx)
    ...
    def run_processing(self):
        ...
        for mol_idx, mol in enumerate(self.mol_set):
            smiles = Chem.MolToSmiles(mol, allHsExplicit=True)

            noH_mol = Chem.RemoveHs(mol)
            conformer_indices = self.__gen_conformers__(mol, n_cpu=self.n_cpu, numConfs=self.max_conformer_per_isomer, maxAttempts=self.max_attempts)

            if len(conformer_indices) == 0:
                continue

            self.__correct_cis_amide__(mol, conformer_indices)

            minimized_energies = AllChem.MMFFOptimizeMoleculeConfs(mol, mmffVariant='MMFF94s', maxIters=5000, numThreads=self.n_cpu)
            ...
            if len(saved_conformer_indices) > 0:
                saved_sdf_file_name = self.__write_conformers_to_sdf__(self.sdf_path, mol, saved_conformer_indices, minimized_energies, self.mol_name, mol_idx)
