...
#Chem.MolFromMolFile('/backup/wei/tmp/rnn/'+str(name_rad)+str(name_time)+'.sdf')
        ...
        #        rmol=Chem.Mol(self.template_mol)
        #        tmol=Chem.Mol(mol)
        #        shape_score=pyshapeit.AlignMol(rmol,tmol)
        ...
        for idx,mol in enumerate(multi_temps1):
            ...
            rawFeats = self.fdef.GetFeaturesForMol(tmol)
            featList = [f for f in rawFeats if f.GetFamily() in self.keep]
            try :
                pc_score = self.reference_fms.ScoreFeats(featList) / self.reference_fms.GetNumFeatures()
                score=pc_score * self.reward_weight[2]+ shape_score*self.reward_weight[3]
                ...
                print('RuntimeError')
                score=self.reward_weight[1]+self.reward_weight[0]
                continue
